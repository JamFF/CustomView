...
public class VLayoutFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container,
        ...
        if (getContext() != null) {
            VirtualLayoutManager vlm = new VirtualLayoutManager(getContext());
            recyclerView.setLayoutManager(vlm);

            RecyclerView.RecycledViewPool viewPool = new RecyclerView.RecycledViewPool();
            recyclerView.setRecycledViewPool(viewPool);
            // 设置对应viewType类型的Item的最大缓存数量
            viewPool.setMaxRecycledViews(0, 10);
            // mRecyclerView.setAdapter(new BannerAdapter(getContext()));

            BaseDelegateAdapter bannerAdapter = new BaseDelegateAdapter(new LinearLayoutHelper(),
                    R.layout.vlayout_banner, 1, 1) {
                @Override
            ...
            // 控制子元素之间的水平间距
            GridLayoutHelper gridLayoutHelper = new GridLayoutHelper(5, 10, 0);
            gridLayoutHelper.setPadding(0, 16, 0, 0);

            BaseDelegateAdapter menuAdapter = new BaseDelegateAdapter(gridLayoutHelper,
                    R.layout.vlayout_menu, 10, 2) {
                @Override
                public void onBindViewHolder(@NonNull final BaseViewHolder holder, final int position) {
                    holder.setText(R.id.tv_menu_title_home, ITEM_NAMES[position] + "")
                            .setImageResource(R.id.iv_menu_home, IMG_URLS[position])
                            .getView(R.id.ll_menu_home).setOnClickListener(new View.OnClickListener() {
                        @Override
            ...
            BaseDelegateAdapter newsAdapter = new BaseDelegateAdapter(new LinearLayoutHelper(),
            ...
            DelegateAdapter delegateAdapter = new DelegateAdapter(vlm, true);
            delegateAdapter.addAdapter(bannerAdapter);
            delegateAdapter.addAdapter(menuAdapter);
            delegateAdapter.addAdapter(newsAdapter);
            for (int i = 0; i < ITEM_URL.length; i++) {
                final int finalI = i;
                BaseDelegateAdapter titleAdapter = new BaseDelegateAdapter(new LinearLayoutHelper(),
                        R.layout.vlayout_title, 1, 4) {
                    @Override
                    public void onBindViewHolder(@NonNull BaseViewHolder holder, int position) {
                        ImageView iv = holder.getView(R.id.iv);
                        Glide.with(Objects.requireNonNull(getContext()))
                                .load(ITEM_URL[finalI]).into(iv);
                    }
                };
                GridLayoutHelper gridHelper = new GridLayoutHelper(2);
                BaseDelegateAdapter gridAdapter = new BaseDelegateAdapter(gridHelper,
                    ...
                    public void onBindViewHolder(@NonNull final BaseViewHolder holder, final int position) {
                        ImageView iv = holder.getView(R.id.iv);
                        Glide.with(Objects.requireNonNull(getContext()))
                                .load(GRID_URL[position]).into(iv);
